...
如果没有动态配置集群(默认的分布式配置环境)，则不会广播所有\ascii{Worker}动态地创建\code{WorkerSession}。事实上，每个\ascii{Worker}都存在一个\code{SessionMgr}实例，它持有一个名为\code{legacy\_session\_}的\code{WorkerSession}实例。因此，每个\ascii{Worker}存在一个全局唯一的\code{WorkerSession}实例。
...
如\refig{dist-worker-get-status}所示，在创建\code{MasterSession}之前，\code{MasterSession}会尝试\ascii{ping}所有的\code{Worker}实例，获取远端所有\code{Worker}所有的计算设备信息。但是，为了获取远端设备集，需要解决两个基础性的问题。

\begin{enum}
  \eitem{获取集群中所有\code{Worker}的名字列表；}
  \eitem{根据\code{Worker}的名字创建\code{WorkerInterface}实例；其中，\code{WorkerInterface}是访问远端\code{Worker}上的\code{WorkerSevice}的本地接口。}
\end{enum}
...
\code{GrpcChannelCache}用于获取或创建集群中所有远端\code{Worker}的\ascii{gRPC}通道。其中，\code{ListWorkers}用于返回集群中所有的\code{Worker}的名称列表。\code{TranslateTask}用于将\code{Worker}的名称转换为\code{host:port}的地址信息。\code{FindWorkerChannel}则用于从缓存中查找\code{grpc::Channel}实例；如果未找到，则根据地址信息动态创建一个\code{grpc::Channel}实例。
...
如\refig{dist-grpc-channel-cache}所示，\code{GrpcChannelCache}创建\code{grpc::Channel}的查找过程使用了缓存技术，避免每次实时创建\code{grpc::Channel}实例的开销。当缓存中查找失败时，通过调用\code{FindChannelOnce}延迟地动态创建\code{grpc::Channel}实例，并添加至缓存中。
...
另外，\code{GrpcChannelCache}的类层次结构遵循隐式的“树型”结构，\code{SparseGrpcChannelCache}是树节点，它为某个\ascii{Job}创建相应的\code{grpc::Channel}实例集；其中，每个\ascii{Task}对应一个\code{grpc::Channel}。
...
而\code{MultiGrpcChannelCache}通过\code{caches\_}持有多个\code{SparseGrpcChannelCache}实例，实现整个集群所有\ascii{Worker}节点的\code{grpc::Channel}的组合创建。为了进一步提高\code{grpc::Channel}实例的查找过程，\code{MultiGrpcChannelCache}缓存了访问过的\code{SparseGrpcChannelCache}实例。
...
实现通过\code{DeviceFinder::GetRemoteDevices}获取远端设备集。它的工作原理非常简单，它根据\code{GrpcWorkerCache::ListWorkers}获取集群中所有的\ascii{Worker}的名字列表；然后，根据\code{worker\_name}的名称，通过调用\code{GrpcWorkerCache::CreateWorker}创建访问远端\ascii{Worker}服务的\code{WorkerInterface}实例。最后，通过\code{WorkerInterface}向远端\ascii{Worker}列表广播发送\code{GetStatusRequest}请求消息，从而实现远端设备集的获取。

为了控制收齐多个\code{Worker}的\code{GetStatusResponse}消息，此处使用了\code{num\_pending\_}的计数器，它初始值为\code{Worker}的数目，由\code{DeviceFinder::Start}设置。当收到一条来自每个\code{Worker}的\code{GetStatusResponse}消息，则回调\code{DeviceFinder::WhenDone}函数，将计数器减\ascii{1}。当计数器减到\ascii{0}时，通过调用\code{pending\_zero\_.notify\_all}，呼醒\code{pending\_zero\_.wait\_for}，随后便可以通过\code{finder.GetRemoteDevices}获取查询结果了。

其中，在\code{DeviceFinder::Start}中，通过\code{NewRemoteDevices}向所有\code{Worker}广播\code{GetStatusRequest}消息，查询相应的设备信息，下一节将重点描述其实现过程。
...
温馨提示，当\code{num\_pending\_}计数器不为零时，则主线程周期性睡眠\code{10}秒中，醒来时发现还是没有收齐所有\ascii{Worker}的响应消息，则打印哪些\ascii{Worker}迟迟没有响应。当看到循环地打印信息时，此时你应该明白\code{/job:worker/task:2}对应的\code{Server}是否异常退出了，或\ascii{Master}与相应的\ascii{Worker}之间网络是否发生了异常等，需要根据具体情况分析和处理。
