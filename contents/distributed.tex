...
如\refig{cc-server-model}所示，\code{GrpcServer}可以扮演\ascii{Master}的角色，对外提供\code{MasterService}服务；其中，它为每一个接入的\ascii{Client}生成唯一的\code{MasterSession}实例。

\code{GrpcServer}可以扮演\ascii{Worker}的角色，对外提供\code{WorkerService}服务；其中，每个\ascii{Worker}可以为多个\ascii{Master}提供计算服务，它为每个向它请求计算服务的\ascii{MasterSession}生成唯一的\code{WorkerSession}实例，等待相应的\ascii{MasterSession}下发计算图的注册和执行命令。

整个\code{GrpcServer}实例承载于\code{grpc::Server}进程之上，它监听特定端口的消息，当消息到达时自动派发到\code{MasterService}或\code{WorkerService}中的回调函数处理。
...
也就是说，\tf{}的架构是可以灵活地扩展。例如，通过扩展\code{protocol}支持新的通信协议，只需扩展实现基于新协议的\code{Server}实例。
...
如\refig{cc-server-interact-1}所示，当客户端接入其中一个\ascii{Server}，此时它扮演了\ascii{Master}的角色，其他\ascii{Server}则扮演了\ascii{Worker}的角色。
...
如\refig{cc-server-interact-2}所示，在分布式的深度学习模式训练过程中，可能存在多个\ascii{Client}分别接入不同的\ascii{Server}实例。此时，\ascii{Client}接入的\ascii{Server}实例扮演了\ascii{Master}角色；但是，该\ascii{Server}实例，相对于集群中另外的\ascii{Server}实例，则扮演了\ascii{Worker}角色。
...
依次类推，在同一个\ascii{Server}内，\ascii{Master}与\ascii{Worker}部署在同一进程内。此时，\ascii{Master}与\ascii{Worker}之间直接使用函数调用。

\subsection{状态机}

如\refig{dist-grpc-server-state-machine}所示，\code{GrpcServer}是一个基于\code{grpc::Server}的服务器，它管理和维护了一个简单的状态机。

其中，在\code{New}状态上启动了\code{grpc::Server}服务，在\code{Started}状态上对外提供\code{MasterService}和\code{WorkerService}的\code{RPC}消息服务，并最终在\code{Stopped}状态下停止\code{MasterService}和\code{WorkerService}服务。
...
此刻，\code{grpc::Server}已经启动，但暂时没有对外提供\code{MasterService}服务和\code{WorkerService}服务。通过调用\code{GrpcServer::Start}方法后，\code{GrpcServer}的状态从\code{New}迁移\code{Started}状态，并启动了两个独立的线程，分别启动\code{MasterService}和\code{WorkerService}的消息处理器。
...
上述过程，通过调用\code{GrpcServer::Join}方法实现，当\code{GrpcServer}处于\code{Started}或\code{Stoped}状态，它将自动调用\code{Thread}的析构函数。
...
\code{MasterService}是一个\ascii{RPC}服务。当\ascii{Client}根据\code{target}接入\ascii{Server}实例后，\ascii{Server}扮演了\ascii{Master}的角色，对外提供\code{MasterService}服务。其中，\ascii{Client}与\ascii{Master}之间的交互遵循\code{MasterService}定义的接口规范。
...
如果\ascii{Worker}之间存在数据依赖，则通过进程间通信完成交互。其中，\ascii{Master}与\ascii{Worker}之间，\ascii{Worker}与\ascii{Worker}之间的交互遵循\ascii{Worker Service}定义的接口规范。
...
如\refig{dist-worker-interaction}所示，\ascii{Client}通过\code{MasterInterface}获取远端\ascii{MasterService}的服务。

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/dist-worker-interaction.png}
\caption{获取\code{MasterService}的RPC过程}
 \label{fig:dist-worker-interaction}
\end{figure}

其中，\code{GrpcRemoteWorker}是\ascii{gRPC}客户端的一种实现，它最终通过\code{Stub}获取远端\ascii{Worker}上的\code{GrpcWorkerService}服务，使得其行为表现得犹如本地函数调用一样。其中，\code{GrpcWorkerService}实现了\code{WorkerService}定义的所有服务接口。

\begin{remark}
从严格意义上讲，\script{GrpcRemoteWorker}是\ascii{Master}或者对端\ascii{Worker}实现的一部分。
\end{remark}

需要注意的是，在本地模式中，显式地增加了\code{MasterInterface}接口的子类型\code{LocalMaster}；而\code{WorkerInterface}接口并不存在\code{LocalWorker}的子类型，而是存在\code{GrpcWorker}的子类型。也就是说，在本地模式中，通过\code{GrpcWorker}的函数调用，直接获取到了\ascii{WorkerService}的服务，不需要额外的函数转调过程。
...
接下来，将详细看看\code{WorkerService}各个接口的消息定义。其中，最重要的就是识别出各个服务的标识。当创建\code{WorkerSession}时，\ascii{Master}携带\code{MasterSession}的标识传递给\ascii{Worker}，实现\ascii{MasterSession}统一管理多个隶属的\code{WorkerSession}实例。

当\ascii{Worker}完成\code{RegisterGraph}后，向\ascii{Master}返回唯一的\code{graph\_handle}，以此标识该图实例。因此，在集群内，可以使用\code{(session\_handle, graph\_handle)}二元组唯一标识该图实例。
...
在同一个集群中，对于一个\code{MasterSession}实例，其他\ascii{Worker}收到相同的\code{session\_handle}。如此，该\ascii{MasterSession}实例便能统一管理隶属于它的所有\code{WorkerSession}实例了。
...
如\refig{dist-worker-register-graph}所示，\code{RegisterGraphRequest}消息中携带\code{MasterSession}分配的\code{session\_handle}，及其子图实例\ascii{graph\_def}。

当\ascii{Worker}完成子图注册及其初始化后，向\ascii{Master}返回该子图的\code{graph\_handle}。
...
如\refig{dist-worker-deregister-graph}所示，当\ascii{Worker}节点上的子图不再需要时(例如，计算图被重新调度，图中节点被重新编排)，此时\ascii{Master}向\ascii{Worker}发送\code{DeregisterGraph}消息，以便注销掉该子图实例。
...
迭代执行\ascii{Worker}节点上注册的子图时，为了区分不同\code{step}，\ascii{Master}生成唯一\code{step\_id}并传递给各个\ascii{Worker}，各个\ascii{Worker}通过\code{step\_id}实现协同。
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{figures/dist-create-session.png}
\caption{创建\code{MasterSession}}
 \label{fig:dist-create-master-session}
\end{figure}

\subsection{GrpcSesion::Create(graph\_def)}
...
\subsection{GrpcRemoteMaster::CreateSession}
...
\subsection{GrpcMasterService::CreateSessionHandler}
...
\subsection{Master::CreateSession}
...
\subsection{MasterSession::Create(graph\_def)}
...
\subsubsection{创建\code{WorkerSession}}
